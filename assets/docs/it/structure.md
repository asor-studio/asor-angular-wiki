...
*   **`isPersistent`** (`boolean`):
    *   `true`: La cache sopravvive al reload della pagina (salvata nel `sessionStorage` del browser).
    *   `false`: La cache è in memoria (RAM) e viene persa al refresh o alla chiusura del tab.
